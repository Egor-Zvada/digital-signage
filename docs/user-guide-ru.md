...
проверенное 28 августа 2026 года. Версия браузерного плеера — `0.3.0-browser`.
...
- Звук без действия пользователя гарантирован только при запуске kiosk Chromium с
  ...
  звука и предлагает один раз нажать для его включения.
...
возвращает `NotAllowedError`, ролик немедленно повторно запускается без звука, а
снизу появляется небольшая кнопка для однократного разрешения звука. На постоянном
kiosk-клиенте Chromium нужно запускать с
...
4. При необходимости изменить длительность фото/сайта/сцены, расписание,
